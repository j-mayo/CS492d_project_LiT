...
def main():
    ...
    light_encoder.to(accelerator.device, dtype=torch.float32) # trainable params are in float32
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                src_latents = vae.encode(batch["src_img"].to(dtype=weight_dtype)).latent_dist.sample()
                src_latents = src_latents * vae.config.scaling_factor

                # Convert tgt images to latent space
                tgt_latents = vae.encode(batch["tgt_img"].to(dtype=weight_dtype)).latent_dist.sample()
