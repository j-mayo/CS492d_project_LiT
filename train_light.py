...
from models.encoder import LightingEncoder
...
def log_validation(
    ...
    with autocast_ctx:
        def gen_img(img, cond, pose):
            lat = vae.encode(norm_img(img)).latent_dist.sample()
            lat = lat * vae.config.scaling_factor
            tgt_encoder_hidden_states = light_encoder(cond, pose)
            gen = net(lat, tgt_encoder_hidden_states)
        ...
        if val_dataloader is not None:
            ...
            images = gen_img(src_img[:val_batch_size],
                             batch["tgt_condition"][:val_batch_size],
                             batch["pose"][:val_batch_size])
            pose_variations = gen_img(src_img[:1].repeat(val_batch_size, 1, 1, 1),
                                      random_cond,
                                      batch["pose"][:1].repeat(val_batch_size))
...
def main():
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net, light_encoder):
                ...
                model_pred = net(src_latents, tgt_encoder_hidden_states)
    ...
    if accelerator.is_main_process:
        ...
        torch.save(net.state_dict(), opj(args.output_dir, 'net.pth'))
        #torch.save(vae.state_dict(), opj(args.output_dir, 'vae.pth'))
        torch.save(light_encoder.state_dict(), opj(args.output_dir, 'light_encoder.pth'))
