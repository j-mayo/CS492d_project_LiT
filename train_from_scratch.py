...
def log_validation(
    ...
    with autocast_ctx:
        def gen_img(img, cond, pose):
            images = pipeline(
                    image=img,
                    light_conditioning=cond,
                    pose_conditioning=pose,
        ...
        if val_dataloader is not None:
            for batch in val_dataloader:
                ...
                images = gen_img(src_img, batch["tgt_condition"], batch["pose"])
...
def main():
    ...
    #for name, module in light_encoder.named_children():
    #    cprint(f"Layer Name: {name}", color='cyan')
    #    cprint(f"Layer Module: {module}\n", color='cyan')
    light_encoder.train()
    #vae = AutoencoderKL.from_config(args.pretrained_model_name_or_path,
    #                                subfolder="vae")
    ...
    # add light encoder
    unet, light_encoder, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        unet, light_encoder, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet, light_encoder):
                ...
                tgt_encoder_hidden_states = light_encoder(tgt_cond, batch["pose"])
                ...
                if accelerator.sync_gradients:
                    params_to_clip = unet_parameters + light_encoder_parameters
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        #torch.save(unet.state_dict(), opj(save_path, 'unet.pth'))
                        #torch.save(vae.state_dict(), opj(save_path, 'vae.pth'))
                        #torch.save(light_encoder.state_dict(), opj(save_path, 'light_encoder.pth'))
                        accelerator.save_state(f"{save_path}/pytorch_model.bin")
            ...
            if (
                ...
                with torch.no_grad():
                    ...
                    pipeline = DiffRelight(
                        ...
                        #light_encoder=unwrap_model(light_encoder),
    ...
    if accelerator.is_main_process:
        unet, vae, light_encoder = unet.to(torch.float32), vae.to(torch.float32), light_encoder.to(torch.float32)

        save_path = opj(args.output_dir, f"checkpoint-{global_step}")
        accelerator.save_state(f"{save_path}/pytorch_model.bin")

        torch.save(unet.state_dict(), opj(args.output_dir, 'unet.pth'))
        #torch.save(vae.state_dict(), opj(args.output_dir, 'vae.pth'))
        torch.save(light_encoder.state_dict(), opj(output_dir, 'light_encoder.pth'))
