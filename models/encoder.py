...
from modules import ConvReduceModule, ConvNet, AttnDownNet
...
class LightingEncoder(AbstractEncoder):
    def __init__(self, num_layers, input_channels, embedding_dim, num_poses, light_pos_maps=None, device="cuda"):
        ...
        #self.light_module = ConvReduceModule(num_layers=num_layers,
        #                                     input_channels=input_channels,
        #                                     embedding_dim=embedding_dim,
        #                                     num_poses=num_poses).to(device)
    ...
    def lookup_light_condition(self, label, base_label=None):
        if base_label is None: base_label = 13 * torch.ones_like(label, dtype=torch.long)
        lcond_tgt = self.light_condition_table[label.to(self.light_condition_table.device).long()-1]
        lcond_src = self.light_condition_table[base_label.to(self.light_condition_table.device)-1]
        return self.lcond_pooler(lcond_tgt - lcond_src)
